...
def test_create_baseball_player_score(baseball_player_score_repository):
    ...
    new_score = BaseballPlayerScore(
        ...
        player_team="Team A",
        score=10,
        match_date=datetime.strptime("2023-01-01", "%Y-%m-%d").date()
    )
    ...
    assert result.entity.match_date == (
...
def test_get_by_id(baseball_player_score_repository):
    ...
    # Act
    retrieved_score = (
        baseball_player_score_repository.get_by_id(created_score.id)
    )
...
def test_get_by_id_not_found(baseball_player_score_repository):
    ...
    assert retrieved_score.error_message == (
        f"Entity with id {score_id} not found"
    )
...
def test_delete(baseball_player_score_repository):
    ...
    created_score = (
        baseball_player_score_repository.create(score_to_delete).entity
    )

    # Act
    delete_result = (
        baseball_player_score_repository.delete(created_score.id)
    )
    ...
    assert deleted_score.error_message == (
        f"Entity with id {created_score.id} not found"
    )
...
def test_update(baseball_player_score_repository):
    ...
    created_score = (
        baseball_player_score_repository.create(score_to_update).entity
    )
    update_data = {"score": 30}

    # Act
    update_result = (
        baseball_player_score_repository.update(created_score.id, update_data)
    )

    # Assert
    assert update_result.is_ok
    updated_score = (
        baseball_player_score_repository.get_by_id(created_score.id).entity
    )
...
def test_get_score_from_player_and_date(baseball_player_score_repository):
    ...
    result = baseball_player_score_repository.get_score_from_player_and_date(
        "Test Player",
        datetime.strptime("2023-05-01", "%Y-%m-%d").date()
    )
